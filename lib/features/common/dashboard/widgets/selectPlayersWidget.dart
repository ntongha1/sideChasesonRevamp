...
class SelectPlayersWidget extends StatefulWidget {
  List<String>? teamIds;
  Function onPlayersAdded;

  SelectPlayersWidget({Key? key, this.teamIds, required this.onPlayersAdded})
      : super(key: key);

  @override
  _SelectPlayersWidgetState createState() => _SelectPlayersWidgetState();
}

class _SelectPlayersWidgetState extends State<SelectPlayersWidget> {
  ...
  PlayersInATeamModel? playersInATeamModelAll;
  ...
  Future<void> _getData() async {
    ...
    await serviceLocator<CommonCubit>()
        .getPlayersInATeamList(widget.teamIds![0]);
      ...
      child: BlocListener(
        ...
        listener: (_, state) {
          ...
          if (state is PlayerListSuccess) {
            context.loaderOverlay.hide();
            PlayersInATeamModel p = serviceLocator.get<PlayersInATeamModel>();
            setState(() {
              isLoading = false;
