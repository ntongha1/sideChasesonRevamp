...
import 'package:sonalysis/widgets/team_row_item.dart';
...
class SelectTeamsWidget extends StatefulWidget {
  String? clubId;
  Function onTeamsAdded;

  SelectTeamsWidget({Key? key, this.clubId, required this.onTeamsAdded})
      : super(key: key);

  @override
  _SelectTeamsWidgetState createState() => _SelectTeamsWidgetState();
}
...
class _SelectTeamsWidgetState extends State<SelectTeamsWidget> {
  ...
  bool isLoading = true;
  TeamsListResponseModel? playersInATeamModel;
  TeamsListResponseModel? playersInATeamModelAll;
  List<TeamsListResponseModelData> playersSelected = [];
  List<TeamsListResponseModelData> playersAll = [];
  List<TeamsListResponseModelData> playersFiltered = [];
  ...
  Future<void> _getData() async {
    ...
    await serviceLocator<CommonCubit>().getTeamList(widget.clubId!);

    setState(() {});
  }

  void doSelect(TeamsListResponseModelData player) {
  ...
  void searchNow(String? v) {
    ...
    List<TeamsListResponseModelData> temp = playersAll.where((element) {
      String fname =
        ...
        child: BlocListener(
          ...
          listener: (_, state) {
            ...
            if (state is TeamListSuccess) {
              context.loaderOverlay.hide();
              TeamsListResponseModel p =
                  serviceLocator.get<TeamsListResponseModel>();
              print("list this is :");
              print("fine: " +
                  p.teamsListResponseModelData![0].selected.toString());
              setState(() {
                isLoading = false;
                playersInATeamModelAll = p;
                playersInATeamModel = p;
                playersAll = p.teamsListResponseModelData!;
                playersFiltered = p.teamsListResponseModelData!;
              });
            }
          },
          child: Container(
                                ...
                                onPressed: canGoNext()
                                    ? () {
                                        ...
                                        serviceLocator
                                            ...
                                            .pop();
                                        widget.onTeamsAdded(playersSelected);
                                      }
                                    : null)),
                      ],
                    ))),
